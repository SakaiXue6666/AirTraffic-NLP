...
class Preprocessor(object):
    ...
    def get_ent2token_spans(self, text, entity_list):
        """
        将原始实体字符位置映射为 token 位置，兼容子词拆分情况

        Args:
            text (str): 原始文本
            entity_list (list): [(start, end, ent_type), ...] - 字符级位置，闭区间：[start, end]

        Returns:
            list: [(token_start, token_end, ent_type), ...] - token级位置，闭区间：[start, end]
        """
        ent2token_spans = []
        ...
        for ent_start_char, ent_end_char, ent_type in entity_list:
            # 改为闭区间转开区间：end_char + 1
            ent_start = ent_start_char
            ent_end = ent_end_char + 1

            token_start = token_end = None
            for idx, (token_char_start, token_char_end) in enumerate(token2char_span_mapping):
                if token_char_start <= ent_start < token_char_end:
                    token_start = idx
                if token_char_start < ent_end <= token_char_end:
                    token_end = idx
                    break

            # 如果仍未能找到结束位置，再向后找
            if token_start is not None and token_end is None:
                for idx in range(token_start + 1, len(token2char_span_mapping)):
                    _, token_char_end = token2char_span_mapping[idx]
                    if token_char_end >= ent_end:
                        token_end = idx
                        break

            if token_start is not None and token_end is not None:
                ent2token_spans.append((token_start, token_end, ent_type))
            else:
                # 无法定位实体 token span，跳过
                continue
